...
class DefaultMovieRemoteDatasource @Inject constructor(@Dispatcher(Dispatchers.IO) private val dispatcher: CoroutineDispatcher, private val apiService: ApiService) : MovieRemoteDatasource {
    ...
    override suspend fun getPopularMovies(page: Int): Result<List<MovieItemDto>> = withContext(dispatcher) {
        safeApiCall({ apiService.getPopularMovies(page = page) }) { it.results }
    }
    ...
    override suspend fun discoverMovies(genre: String): Result<List<MovieItemDto>> = withContext(dispatcher) {
        safeApiCall({ apiService.discoverMovies(genre = genre) }) { it.results }
    }
}
